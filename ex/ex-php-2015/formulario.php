...
<body>
  ...
  <div id="formulario">
    ...
    $ingre=isset($_POST["ingre"])?$_POST["ingre"]:array();
    ...
    if ($_POST) {
          if($nom=="") $faltanCampos++;
          if($dir=="") $faltanCampos++;
          if(count($ingre)<1) $faltanCampos++;
          if($tam=="") $faltanCampos++;
          if($fpago=="") $faltanCampos++;
          if($ins=="") $faltanCampos++;

          if($faltanCampos>0) {
            echo "<p class='noaceptado'>Faltan campos por cubrir</p>";

          }
          else {
            echo "<p class='aceptado'>Formulario aceptado</p>";
          $ficheiro=@fopen("formulario.txt", "a") or die("<p>Non foi posible abrir o ficheiro datos.csv</p>"); 
          $depIngre=implode("", $ingre);
          fputs($ficheiro,"$nom;$dir;$depIngre;$tam;$fpago;$ins\n");
          fclose($ficheiro);
